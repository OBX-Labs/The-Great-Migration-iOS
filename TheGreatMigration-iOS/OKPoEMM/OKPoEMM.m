...
static NSString *PERFORMANCE_PASSWORD = @"11615";
...
- (id) initWithFrame:(CGRect)aFrame EAGLView:(UIView*)aEAGLView isExhibition:(BOOL)flag
{
    ...
    if (self)
    {
        // List available fonts (helper)
        //[[OKAppProperties sharedInstance] listAvailableFonts];
        
        // Will not prompt info view if set to true
        isExhibition = flag;
...
{
    [infoView setSelectedIndex:menuTab];
    [self presentInfoViewCntroller];
}
...
{
    [self setisExhibition:flag];
    
    // Hide menu buttons if shown
...
#warning your EAGLView should be looking for this value (isPerformance) in update to flip a flag that will start the guide
    
    [[NSUserDefaults standardUserDefaults] setBool:flag forKey:@"isPerformance"];
    [[NSUserDefaults standardUserDefaults] synchronize];
}

- (void) promptForPerformance
{
    if(!hasPrompted && ![[NSUserDefaults standardUserDefaults] boolForKey:@"isPerformance"]) {
        hasPrompted = YES;
        
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Performance Version" message:nil delegate:self cancelButtonTitle:@"Dismiss" otherButtonTitles:@"Continue", nil];
        [alert setAlertViewStyle:UIAlertViewStyleSecureTextInput];
        [alert show];
    }
    else {
        if([[NSUserDefaults standardUserDefaults] boolForKey:@"isPerformance"]) [self setIsPerformance:YES];
    }
}
...
- (BOOL) alertViewShouldEnableFirstOtherButton:(UIAlertView *)alertView
{
    ...
    if( [inputText length] > 0 ) return YES;
    else return NO;
}
...
- (void) alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if([title isEqualToString:@"Continue"])
    {
        ...
        if([password.text isEqualToString:PERFORMANCE_PASSWORD]) {
            [infoView dismissViewControllerAnimated:YES completion:nil];
            [self setIsPerformance:YES];
        } else {
            [self setIsPerformance:NO];
        }
    }
}
...
- (void) presentInfoViewCntroller
{
    ...
    if(lB.alpha == 1.0 || rB.alpha == 1.0) [self toggleVisible];
}
...
// Helper to determine if corner was touched
- (BOOL) didTouchHotCorner:(CGPoint)aPoint
{
    //VICTOR - PUT BACK HOT CORNERS
