...
// Loads the default poem attached with the app
        ...
        for(NSString *packageKey in packageKeys)
        {
            ...
            for(NSDictionary *textDict in package)
            {
                ...
                if(fontFile) {
                    ...
                    // Just look for the .fnt file, since we know both will exist
                    NSString *retinaPath = [[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"%@@2x", fontFile] ofType:@"fnt"];
                    if([[NSFileManager defaultManager] fileExistsAtPath:retinaPath] && [OKPoEMMProperties isRetina])
                        fontFile = [fontFile stringByAppendingString:@"@2x"];
                ...
                if(fontOutlineFile) {
                    ...
                    // Just look for the .fnt file, since we know both will exist
                    NSString *retinaPath = [[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"%@@2x", fontOutlineFile] ofType:@"fnt"];
                    if([[NSFileManager defaultManager] fileExistsAtPath:retinaPath] && [OKPoEMMProperties isRetina])
                        fontOutlineFile = [fontOutlineFile stringByAppendingString:@"@2x"];
...
- (BOOL) setTextList:(NSDictionary *)aDict andSave:(BOOL)aSave
{
    ...
    for(NSString *packageKey in packageKeys)
    {
        ...
        for(NSDictionary *textDict in package)
        {
            ...
            // Image file doesn't exists, download it
            if(!imageFileExists) {
            ...
            // No properties have been found, continue
            ...
            if(fontFile) {
                ...
                if([OKPoEMMProperties isRetina] && [OKPoEMMProperties doesSupportsRetinaFonts])
                    fontFile = [fontFile stringByAppendingString:@"@2x"];
...
// Counts the amount of packages
- (int) packagesCount { return [[textList allKeys] count]; }

// Gets the package key at index
...
// Returns the text dict for given text id at index
...
// Checks the status of cache folders, creates the author and font folders if needed
...
// Checks the author folder, if it doesn't exist, it creates it
...
// Copies files from bundle to cache
...
// Gets the path to the properties
+ (NSString*) listPath
{
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
...
// Clears the cache, this is in case of an epic fail, we assume something went terribly wrong and we want to restart
+ (void) clearCache
{
...
// Copies files from server to cache
...
// Creates a folder for a given name
