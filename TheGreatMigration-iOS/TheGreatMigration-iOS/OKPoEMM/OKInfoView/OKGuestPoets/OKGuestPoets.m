...
static float SEPERATOR_TINT[] = {0.35f, 0.35f, 0.35f, 1.0f}; // This should not change, it sets the same color as the table view seperator color
...
- (void) buildHeadersForTableView
{
    ...
    for(int i = 0; i < [[OKTextManager sharedInstance] packagesCount]; i++)
    {
        ...
        if(![[packages allKeys] containsObject:[textDict objectForKey:@"Author"]]) // Has not yet been created
        {
            ...
            NSString *imagePath = [[OKTextManager authorPath:author] stringByAppendingPathComponent:[textDict objectForKey:@"AuthorImage"]];
...
- (BOOL) hasConnection
{
    NSString *networkStatus = [NSString stringWithContentsOfURL:[NSURL URLWithString:@"http://www.apple.com"] encoding:NSASCIIStringEncoding error:nil];
...
- (void) updateTexts
{
    // So that it doesn't update on each appear
    hasUpdatedOnce = YES;
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[[NSOperationQueue alloc] init] completionHandler:^(NSURLResponse *response, NSData *data, NSError *error)
    {
        if([data length] > 0 && error == nil)
        {
            ...
            #warning Comment or remove this line when testing is done
            ...
            {                                
                [[OKTextManager sharedInstance] setTextList:plist andSave:YES];
            }
