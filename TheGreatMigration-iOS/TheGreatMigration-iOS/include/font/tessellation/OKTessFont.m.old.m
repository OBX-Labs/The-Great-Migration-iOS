...
- (void) parseFont:(NSString*)controlFile xmlTess:(CXMLDocument*)xmlTess;
- (void) parseCommon:(NSString*)line;
- (void) parseCharacterDefinition:(NSString*)line charDef:(OKCharDef*)CharDef xmlTess:(CXMLDocument*)xmlTess;
...
- (id) initWithControlFile:(NSString*)controlFile scale:(float)fontScale filter:(GLenum)filter {
	...
	if (self != nil)
    {
        ...
        CXMLDocument *xmlTess; xmlTess = [[CXMLDocument alloc] initWithData:XMLData options:0 error:nil];
        
        // Parse the control file and populate charsArray which the character definitions	
        //[self parseTess:controlFile];
		[self parseFont:controlFile xmlTess:xmlTess];
...
- (void) parseFont:(NSString*)controlFile xmlTess:(CXMLDocument*)xmlTess
{
	...
	while(line = [nse nextObject]) {
		...
		if([line hasPrefix:@"common"]) {
			...
			OKCharDef *characterDefinition = [[OKCharDef alloc] initCharDefWithFontScale:scale];
			[self parseCharacterDefinition:line charDef:characterDefinition xmlTess:xmlTess];

            // Make sure the slot is clean
            [charsArray[[characterDefinition charID]] release];
            
			// Add the CharDef returned to the charArray
			charsArray[[characterDefinition charID]] = characterDefinition;
            [charsArray[[characterDefinition charID]] retain];
...
- (void) parseCharacterDefinition:(NSString*)line charDef:(OKCharDef*)characterDefinition xmlTess:(CXMLDocument*)xmlTess {
...
- (float) getWidthForChar:(int)aChar {
	return [charsArray[aChar] xAdvance] * scale;
}
...
- (OKCharDef*) getCharDefForChar:(NSString*)aChar
{
    ...
    for(int i=0; i<[aChar length]; i++)
    {
        ...
        charDef = [[charsArray[charID] copy] autorelease];
    }
    
    return charDef;
}
- (OKCharDef*) getCharDefForCharInt:(int)aChar
{
    OKCharDef *charDef;
        
    // Grab the unicode value of the current character
    charDef = [[charsArray[aChar] copy] autorelease];
...
- (NSArray*) getCharDefForString:(NSString*)aString
{
	...
	for(int i=0; i<[aString length]; i++)
    {
        ...
        OKCharDef *charDef = [[charsArray[charID] copy] autorelease];
